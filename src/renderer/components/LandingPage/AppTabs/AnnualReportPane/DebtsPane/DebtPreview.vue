  ...
  <b-container fluid>
    <b-form ref="form" v-on:submit="onSubmit" novalidate no-validation>
      ...
      <b-row>
        ...
        <b-col>
          <b-form-group>
            <b-form-input v-on:keypress.enter="adaptAutoNumericAmount('amount')" v-on:cut="updateAfterCut" id="amountInput" type="text" v-model="form.amount" v-on:mouseleave="disableValueTooltip ? null : hideTooltip('amountInput')" v-bind:class="{ 'is-invalid': shouldValidate && missingValue }"/>
          </b-form-group>
        </b-col>
      </b-row>
      <b-row>
        <b-col>
          <b-button-group class="float-right">
            <b-button ref="saveDebtBtn" id="saveDebtBtn" v-on:mouseleave="hideTooltip('saveDebtBtn')" type="submit" variant="light" class="btn-lg">
